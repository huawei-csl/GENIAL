...
DEFAULT_BASE = (
    "/netscratch/aisrv02/farnold/proj/GENIAL/output/"
...
def main() -> int:
    ...
    for d in res_dirs:
        ...
        try:
            ...
            df = pd.read_parquet(pq, columns=["run_identifier", "step"])
        ...
        if "run_identifier" not in df.columns or "step" not in df.columns:
            bad_files += 1
            if not args.quiet:
    ...
    print()
    print(text_hist(uniq_hist, title="Histogram: unique run_identifier per flowy_data_record.parquet"))
    ...
    print()
    print(text_hist(maxstep_hist, title="Histogram: max(step) per flowy_data_record.parquet"))
