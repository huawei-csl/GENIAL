...
if [ -e "/app/tmp/primitives_necessary_only.v" ]; then
    mv /app/tmp/primitives_necessary_only.v $WORK_DIR/hw/mydesign/hdl/.
else
    echo "File /app/tmp/primitives_necessary_only.v does not exist. Skipping."
fi
...
cp $WORK_DIR/hw/mydesign/tests/*_db.pqt /app/tmp/synth_results/.
