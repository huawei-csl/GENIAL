...
def derive_diverse_initial_dataset(
    dir_config: ConfigDir, n_samples: int, device: str, batch_size: int = 500, candidates_per_iter: int = 1000
):
    ...
    values_list = list(tc_enc.values())
    ...
    dataset = torch.randperm(values.size(0), generator=g).unsqueeze(0).to(torch.int8)
    ...
    while count < n_samples:
        ...
        permutations = torch.stack(
            [torch.randperm(values.size(0), generator=g) for _ in range(candidates_per_iter)]
        ).to(torch.int8)
    ...
    for i in range(dataset.shape[0]):
        ...
        for j, k in enumerate(keys):
            temp_dict[k] = values_list[dataset[i, j]]
