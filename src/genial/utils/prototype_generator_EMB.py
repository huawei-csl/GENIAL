...
from genial.utils.utils import get_twos_complement_tensor

plt.rcParams["font.size"] = 20


# Placeholder for patch_prototypes to satisfy linters; functionality not provided in EMB variant.
def patch_prototypes(prototype_data_path: Path, dir_config: ConfigDir, fix_strategy_version: int = 3):
    raise NotImplementedError("patch_prototypes is not available in this variant.")
...
def generate_prototype_patterns(
    ...
    **kwargs,
    ...
    # Prepare encoded values list (required for embedding)
    ...
    optimizer_choice = dir_config.args_dict.get("optimizer_choice")
    optimizer_lr = dir_config.args_dict.get("optimizer_lr")

    if optimizer_choice == "adam":
        optimizer = torch.optim.Adam(batched_prototypes, lr=optimizer_lr, weight_decay=0.01)
    elif optimizer_choice == "lamb":
        optimizer = Lamb(batched_prototypes, lr=optimizer_lr, weight_decay=0.01)
    elif optimizer_choice == "soap":
        optimizer = SOAP(batched_prototypes, lr=optimizer_lr, weight_decay=0.01)
    else:
        raise ValueError(f"Unrecognized optimizer choice: {optimizer_choice}")
    ...
    final_sinkhorn_temp = dir_config.args_dict.get("final_sinkhorn_temp")
    sinkhorn_temp_factor = dir_config.args_dict.get("sinkhorn_temp_factor")
    sinkhorn_iter_start = dir_config.args_dict.get("sinkhorn_iter_start")
    sinkhorn_iter_end = dir_config.args_dict.get("sinkhorn_iter_end")
        ...
        for epoch_idx in range(max_epochs):
            ...
            for step_idx, prototype_batch in enumerate(batched_prototypes):  # Optimization happens inline
                if epoch_idx < sinkhorn_start_epoch:
                    ...
                    temp = sinkhorn_temp_factor * (final_sinkhorn_temp**frac)  # temperature decay
                    n_iter = int(
                        sinkhorn_iter_start + frac * (sinkhorn_iter_end - sinkhorn_iter_start)
                    )  # T: 3 → 15 iterations
    ...
    data = {"all_prototypes": torch.stack(bit_representations)}
...
def parse_args() -> dict[str, Any]:
    ...
    arg_parser.add_argument(
        "--max_epochs", type=int, default=1500, help="Number of epochs to run input prototype generation for."
    )

    arg_parser.add_argument(
        "--sinkhorn_start_epoch", type=int, default=150, help="Epoch where we switch from softmax to sinkhorn"
    )

    arg_parser.add_argument("--alpha_weight", type=float, default=0.4, help="alpha weight for the sharpness")

    arg_parser.add_argument("--beta", type=float, default=16.5, help="beta weight for the diversity")

    arg_parser.add_argument(
        "--final_sinkhorn_temp", type=float, default=0.15, help="Final temperature for Sinkhorn normalisation"
    )

    arg_parser.add_argument(
        "--sinkhorn_temp_factor", type=float, default=0.75, help="Temperature factor for Sinkhorn normalisation"
    )

    arg_parser.add_argument("--optimizer_lr", type=float, default=0.07, help="optimizer learning rate")

    arg_parser.add_argument("--optimizer_choice", type=str, default="lamb", help="optimizer learning rate")

    arg_parser.add_argument("--sinkhorn_iter_start", type=int, default=4, help="Starting number of Sinkhorn iterations")

    arg_parser.add_argument("--sinkhorn_iter_end", type=int, default=18, help="Ending number of Sinkhorn iterations")
